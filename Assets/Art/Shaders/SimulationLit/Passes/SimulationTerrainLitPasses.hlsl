...
#include "SimulationLibrary.hlsl"
...
struct Varyings
{
    ...
    float3 worldPos : TEXCOORD10; //SIM SHADER
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal, in InputData inputData)
{
    half4 diffAlbedo[4];
    ...
    #ifdef USE_BIPLANAR
    //BIPLANAR CUSTOM
    const float threshold = smoothstep(_BiPlanarSize, _BiPlanarStrength, abs(dot(inputData.normalWS, float3(0, 1, 0))));
    //This basically blends the splat control to the red channel of the splat control,
    //and red is the first layer in the terrain (aka should be dirt)
    splatControl = lerp(half4(1,0,0,0), splatControl, threshold);
    #endif
...
Varyings SplatmapVert(Attributes v)
{
    ...
    //SIM SHADER
    o.worldPos = mul(unity_ObjectToWorld, float4(v.positionOS.xyz, 1));
...
#endif
{
    ...
    SplatmapMix(IN.uvMainAndLM, IN.uvSplat01, IN.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS, inputData);
    ...
    outColor = half4(SimAlgorithm(color, IN.worldPos, inputData.normalWS).rgb, 1.0h);
